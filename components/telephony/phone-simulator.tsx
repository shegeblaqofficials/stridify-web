...
interface PhoneSimulatorProps {
  /** The project / sandbox id used to mint a LiveKit token. */
  projectId: string;
  agentName?: string;
...
export function PhoneSimulator({
  ...
  // ── LiveKit wiring (same flow as the iframe / popup embed) ──
  ...
  // Drive UI state from real Room events.
  useEffect(() => {
  ...
  // Always disconnect the room on unmount so we don't leak sessions.
  useEffect(() => {
    return () => {
  ...
  const handleStartCall = useCallback(async () => {
    ...
    if (state === "calling" || state === "connected") return;
    setState("calling");
    ...
    try {
      ...
      // Connected handler will flip the UI to "connected".
    } catch (err) {
      console.error("[telephony-simulator] connect error:", err);
      setState("ended");
  ...
  const handleToggleMute = useCallback(() => {
    ...
    room.localParticipant.setMicrophoneEnabled(!next).catch((err) => {
      console.error("[telephony-simulator] mute toggle error:", err);
    });
  }, [muted, room]);
